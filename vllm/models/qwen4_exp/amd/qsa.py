...
class Qwen4ExpQSAFlashAttentionImpl(FlashAttentionImpl):
    ...
    def forward_qsa(
        ...
        if key_cache.dtype not in (torch.bfloat16, torch.float16) or (
            ...
            raise NotImplementedError(
                "Qwen4Exp QSA requires matching BF16 (or FP16 on RDNA2) Q/K/V"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if model_config.dtype not in (torch.bfloat16, torch.float16):
            raise NotImplementedError(
                "Qwen4Exp QSA currently requires BF16 (FP16 on RDNA2)"
            )
        ...
        if self.kv_cache_torch_dtype not in (torch.bfloat16, torch.float16):
            raise NotImplementedError(
                "Qwen4Exp QSA requires BF16 (or FP16 on RDNA2) cache storage"
            )
