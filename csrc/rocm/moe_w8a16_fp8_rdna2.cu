...
//
// Fused MoE W8A16-FP8 kernel for AMD RDNA2 (gfx1030).
//
// FP8 (E4M3) weights + fp16 activations, per-tile FP8 -> fp16 via a
// 256-entry LUT in constant memory (no hardware CVT on gfx1030), then
// v_dot2_f32_f16 for the inner dot. fp32 accumulator, packed CAS-64
// atomic-write epilogue (byte-identical to W4A16/W8A16-RDNA2).
//
// Weight storage: float8_e4m3fn [E, K, N] (1 byte per element).
// Per-group fp16 scale and zero (block_shape=[0, group_size]).
//
// Storage stays FP8 (no requant on disk). Dequant is per-tile inside
// the kernel.
//
...
#include "q_gemm_rdna2_common.cuh"  // dot22_8_f, atomic_add_pk4_f16
#include "ops.h"
...
// Mirror W4A16 / W8A16-INT8 shape: 256 threads, 256 K × 1024 N cols per
// block, 4 N cols per thread, K_unroll=32 (4 j-iterations × 8 K-positions).
constexpr int THREADS = 256;
constexpr int BLOCK_KN_SIZE = 256;
constexpr int LDS_PAD = 8;

// 256-entry FP8 -> fp16 LUT. Allocated in constant memory. Built at
// host-side via hipMemcpyToSymbol called from ensure_fp8_lut_populated()
// before the first kernel launch (one-shot, std::call_once guarded).
// The cuh header has the matching extern declaration.
// Definition lives in q_gemm_w8a16_fp8_rdna2.cu so all FP8 kernels in a
// single DSO share one device-linker-visible symbol.

// Build the fp8 -> fp16 LUT on the host. Encoding per OCP FP8 E4M3:
// normal: fp16 = sign | (exp+8)<<10 | mant<<7
// subnormal: renormalize leading bit into fp16 implicit position
// NaN: 0x7E00 | (sign << 15)
static void build_fp8_lut_host(uint16_t* lut) {
  for (int b = 0; b < 256; ++b) {
    ...
    if (exp_bits == 0u) {
      if (mant == 0u) {
        ...
        // Subnormal: fp8 value = mant * 2^-9. Normalize by shifting
        // the leading 1 to bit 9 of the fp16 mantissa (bit 10 is the
        // implicit-1 position; the explicit mantissa bits go below).
        uint32_t m = mant;
        int c = 0;
        while ((m & 0x4u) == 0u) { m <<= 1; c++; }
        // m now has leading 1 at bit 2. Trailing bits below bit 2
        // occupy bits [c-1:0]. Place at fp16 mantissa bits [9:0].
        uint32_t trailing = (mant & ((1u << c) - 1u)) << (10 - c);
        uint32_t fp16_exp = 6u + c;
        fp16_bits = (sign << 15) | (fp16_exp << 10) | trailing;
      }
    } else if (exp_bits == 0xFu) {
      fp16_bits = 0x7E00u | (sign << 15);
    } else {
      uint32_t fp16_exp = exp_bits + 8u;
      fp16_bits = (sign << 15) | (fp16_exp << 10) | (mant << 7);
    }
    lut[b] = (uint16_t)fp16_bits;
  }
}

// One-shot LUT initializer. Thread-safe via std::once_flag.
static std::once_flag g_fp8_lut_init_flag;
static void ensure_fp8_lut_populated() {
  std::call_once(g_fp8_lut_init_flag, []() {
    uint16_t lut[256];
    build_fp8_lut_host(lut);
    hipMemcpyToSymbol(HIP_SYMBOL(k_fp8_to_fp16_lut),
                      lut, sizeof(lut), 0, hipMemcpyHostToDevice);
  });
}

template <int BLOCK_SIZE_M>
__global__ void moe_w8a16_fp8_gemm_kernel(
    const half* __restrict__ a,            // [M, K] fp16
    half* __restrict__ c,                  // [M, N] fp16
    const uint8_t* __restrict__ b_q,       // [E, K, N] FP8 (as bytes)
  ...
  const uint8_t* expert_b = b_q + (int64_t)expert_id * expert_weight_stride;
  ...
  for (int k = offset_k; k < end_k; k += K_unroll) {
    ...
    for (int j = 0; j < 4; ++j) {
      ...
      // Gather 8 K positions for 1 N col from K-major FP8 storage.
      // Storage layout: b_q[expert][k * size_n + n] is at (k, n). The
      // 8 K positions for N col n+kk at K positions k_off..k_off+7
      // are at byte addresses (k_off+i) * size_n + (n+kk) for i=0..7.
      // These are size_n bytes apart (NOT consecutive), so we must
      // gather 8 bytes rather than load a single uint64.
      uint64_t w_packed[4];
      #pragma unroll
      ...
      for (int kk = 0; kk < 4; ++kk) {
        ...
        dequant_fp8_8_fp16_lut(w_packed[kk], sh_v, zh_v, dq[kk]);
      }
...
template <int BLOCK_SIZE_M>
void launch_w8a16_fp8_moe(
    const half* a, half* c, const uint8_t* b_q,
  ...
  dim3 block(THREADS);
  moe_w8a16_fp8_gemm_kernel<BLOCK_SIZE_M><<<grid, block, 0, stream>>>(
...
void dispatch_moe_w8a16_fp8(
    ...
    int expert_scales_stride, int expert_zeros_stride,
    bool mul_topk_weight, int output_topk, cudaStream_t stream) {
  ensure_fp8_lut_populated();
  switch (block_size_m) {
    case 1:
      launch_w8a16_fp8_moe<1>(a, c, b_q, b_scales, b_zeros, topk_w,
          sorted_token_ids, expert_ids, num_tokens_post_padded,
          num_token_blocks, size_m, size_n, size_k, groups, top_k,
          expert_weight_stride, expert_scales_stride, expert_zeros_stride,
      ...
      break;
    case 2:
      launch_w8a16_fp8_moe<2>(a, c, b_q, b_scales, b_zeros, topk_w,
          sorted_token_ids, expert_ids, num_tokens_post_padded,
          num_token_blocks, size_m, size_n, size_k, groups, top_k,
          expert_weight_stride, expert_scales_stride, expert_zeros_stride,
          mul_topk_weight, output_topk, stream);
      break;
    case 4:
      launch_w8a16_fp8_moe<4>(a, c, b_q, b_scales, b_zeros, topk_w,
          sorted_token_ids, expert_ids, num_tokens_post_padded,
          num_token_blocks, size_m, size_n, size_k, groups, top_k,
          expert_weight_stride, expert_scales_stride, expert_zeros_stride,
          mul_topk_weight, output_topk, stream);
      break;
    case 8:
      launch_w8a16_fp8_moe<8>(a, c, b_q, b_scales, b_zeros, topk_w,
          sorted_token_ids, expert_ids, num_tokens_post_padded,
          num_token_blocks, size_m, size_n, size_k, groups, top_k,
          expert_weight_stride, expert_scales_stride, expert_zeros_stride,
          mul_topk_weight, output_topk, stream);
      break;
    default:
      TORCH_CHECK(false, "moe_w8a16_fp8_gemm_rdna2: block_size_m must be "
                  "in {1, 2, 4, 8}; got ", block_size_m);
  }
}
...
void moe_w8a16_fp8_gemm_rdna2(
  ...
  TORCH_CHECK(b_q_weight.dim() == 3, "b_q_weight must be 3D [E, K, N] (FP8)");
  ...
  TORCH_CHECK(a.scalar_type() == torch::kHalf,
              "moe_w8a16_fp8_gemm_rdna2 only supports fp16 activations");
  TORCH_CHECK(b_q_weight.scalar_type() == torch::kFloat8_e4m3fn,
              "b_q_weight must be float8_e4m3fn");
  TORCH_CHECK(a.scalar_type() == b_scales.scalar_type(),
              "b_scales dtype must match a");
  TORCH_CHECK(b_qzeros.numel() == 0 ||
              b_qzeros.scalar_type() == torch::kHalf,
              "b_qzeros must be fp16 or empty (host-converted from FP8)");
  ...
  TORCH_CHECK(size_k % 8 == 0,
              "moe_w8a16_fp8_gemm_rdna2: size_k must be a multiple of 8; got ",
              size_k);
  TORCH_CHECK(size_n % 4 == 0,
              "moe_w8a16_fp8_gemm_rdna2: size_n must be a multiple of 4; got ",
              size_n);
  TORCH_CHECK(groups > 0,
              "moe_w8a16_fp8_gemm_rdna2: groups must be > 0");
  TORCH_CHECK(size_k % groups == 0,
              "moe_w8a16_fp8_gemm_rdna2: size_k (", size_k,
              ") must be divisible by groups (", groups, ")");
  const int group_size = size_k / groups;
  TORCH_CHECK(group_size % 8 == 0,
              "moe_w8a16_fp8_gemm_rdna2: group_size must be a multiple of 8; got ",
              group_size);
  ...
  vllm::w8a16_fp8_rdna2::dispatch_moe_w8a16_fp8(
      (const half*)a.data_ptr(), (half*)c.data_ptr(),
      (const uint8_t*)b_q_weight.data_ptr(),
