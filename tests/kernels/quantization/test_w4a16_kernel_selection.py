...
if current_platform.is_rocm():
    from vllm.platforms.rocm import on_gfx1x, on_gfx10x, on_gfx1100
else:
    on_gfx1100 = on_gfx1x = on_gfx10x = lambda: False  # noqa: E731

# Group sizes the HIP skinny path of RDNAHybridW4A16LinearKernel instantiates.
_HYBRID_GROUP_SIZES = (32, 64, 128)


def _expected_rocm_kernel(weight_type, group_size: int, act_type=torch.float16) -> str:
    """Mirror the ROCm priority order in ``_POSSIBLE_KERNELS[ROCM]``:

    RDNA2 (gfx1030, fp16) -> RDNA3 (gfx1100, symmetric uint4b8) ->
    Hybrid (gfx11/gfx12, or gfx10 if RDNA2 cannot implement) -> Triton.
    """
    if on_gfx10x() and act_type == torch.float16:
        return "RDNA2W4A16LinearKernel"
...
def test_choose_mp_linear_kernel_uint4_asymmetric():
    ...
    kernel_type = choose_mp_linear_kernel(config)
    assert kernel_type.__name__ == _expected_rocm_kernel(
        scalar_types.uint4, 64, act_type=torch.bfloat16
    )
